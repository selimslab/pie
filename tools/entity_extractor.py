...
def extract_entities(text, model="en_core_web_sm"):
    ...
    for entity_type in entity_dict:
        entity_dict[entity_type] = sorted(entity_dict[entity_type], key=lambda x: x[1], reverse=True)
        
    return entity_dict

def main():
    parser = argparse.ArgumentParser(description="Extract named entities from a URL using spaCy")
    parser.add_argument("--url", type=str, required=True, help="URL to extract text from")
    parser.add_argument("--model", type=str, default="en_core_web_sm", help="spaCy model to use")
    parser.add_argument("--min-count", type=int, default=1, help="Minimum occurrence count")
    parser.add_argument("--entity-types", type=str, help="Comma-separated list of entity types")
    ...
    for entity_type, entity_list in entities.items():
        entities[entity_type] = [(text, count) for text, count in entity_list if count >= args.min_count]
